...
async def lifespan(app: FastAPI):
    ...
    if v_status.get("reranker_active"):
        logger.info(f"FlashRank Reranker Active: {v_status.get('reranker_model')}")
    
    # Initialize SQLite Session & Escalation DB
    session_srv = get_session_service()
    await session_srv.init_db()
    logger.info("=" * 60)
